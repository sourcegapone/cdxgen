...
const PLUGIN_MANIFEST_FILE = safeExistsSync(
  join(CDXGEN_PLUGINS_DIR, "plugins-manifest.json"),
)
  ? join(CDXGEN_PLUGINS_DIR, "plugins-manifest.json")
  : undefined;
let pluginManifest;
...
let TRUSTINSPECTOR_BIN = process.env.TRUSTINSPECTOR_CMD;
if (safeExistsSync(join(CDXGEN_PLUGINS_DIR, "trustinspector"))) {
  TRUSTINSPECTOR_BIN = join(
    CDXGEN_PLUGINS_DIR,
    "trustinspector",
    `trustinspector-cdxgen-${platform}-${arch}${extn}`,
  );
}
...
function extractComponentHostPaths(component) {
  ...
  for (const property of component?.properties || []) {
    ...
    if (isHostInspectionPath(normalizedValue)) {
